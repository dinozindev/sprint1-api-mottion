...
public static class SetorEndpoints
{
   public static void MapSetorEndpoints(this IEndpointRouteBuilder app)
   {
      ...
      setores.MapGet("/", async (SetorService service) => await service.GetAllSetores())
      ...
      setores.MapGet("/{id:int}", async ([Description("Identificador único de Setor")] int id, SetorService service) => await service.GetSetorById(id))
